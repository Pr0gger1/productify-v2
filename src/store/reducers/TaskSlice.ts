...
export const deleteSubTaskAsync = createAsyncThunk<
	{ tasks: TasksWithSelected; userId: string },
	DeleteSubTaskParams
>('subtask/delete', async (params: DeleteSubTaskParams, { getState }) => {
	const state = getState() as RootState;
	const { tasks } = state.taskStates;
	const userId: string = state.authStates.userData?.uid as string | '';

	const newTasks: TasksWithSelected = TaskService.deleteSubTask(
		tasks,
		params.taskId,
		params.subTaskId,
	);
...
export const updateTaskAsync = createAsyncThunk<
	{ tasks: TasksWithSelected; userId: string },
	ITask
>('task/update', async (taskData: ITask, { getState }) => {
	const state = getState() as RootState;
	const { tasks } = state.taskStates;
...
export const updateSubTaskAsync = createAsyncThunk<
	{ tasks: TasksWithSelected; userId: string },
	UpdateSubTaskParams
>('subtask/update', async (params: UpdateSubTaskParams, { getState }) => {
	const state = getState() as RootState;
	const { tasks } = state.taskStates;
	const userId = state.authStates.userData?.uid as string | '';
	...
	extraReducers: builder => {
		builder
			.addCase(
				...
				(state, action: PayloadAction<ITasksWithUser>): void => {
			...
			.addCase(updateTaskAsync.rejected, (state, action): void => {
				if ('error' in action && action.error) state.fetchError = action.error;
			})

			.addCase(
				updateSubTaskAsync.fulfilled,
				(
					state,
					action: PayloadAction<{
						tasks: TasksWithSelected;
			...
			.addCase(
				...
				(state, action: PayloadAction<IUserTaskData>): void => {
