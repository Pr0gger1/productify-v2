...
const AuthForm: FC<AuthFormProps> = ({ register = false }): JSX.Element => {
	...
	const onSubmitHandler = async (data: any) => {
		if (register) {
			if (data.password === data.repeatPassword) {
				...
				setToast({
					message:
						'На вашу электронную почту было отправлено письмо с подтверждением',
					type: 'info',
					hideDuration: 5000,
				});
			} else {
	...
	return (
		...
		>
							...
							InputProps={{
								...
								startAdornment: (
									<InputAdornment position="start">
										<BadgeTwoToneIcon sx={{ color: inputColorStyles }} />
									</InputAdornment>
								),
							}}
						/>
				...
				<FormControl>
					<TextField
						...
						InputProps={{
							...
							startAdornment: (
								<InputAdornment position="start">
									<EmailTwoToneIcon sx={{ color: inputColorStyles }} />
								</InputAdornment>
							),
						}}
					/>
				...
				<FormControl>
					<TextField
						...
						InputProps={{
							...
							startAdornment: (
								<InputAdornment position="start">
									<KeyTwoToneIcon sx={{ color: inputColorStyles }} />
								</InputAdornment>
							),
							endAdornment: (
			...
			<Button type="submit" className={styles.login_button} variant="contained">
