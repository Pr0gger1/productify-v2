...
const TaskCategorySection: FC = (): JSX.Element => {
	...
	const isBaseTaskGroup: boolean = Object.values(baseGroupIds).includes(
		selectedTask?.groupId ?? '',
	);

	const taskGroupList: ITaskGroup[] =
		taskGroups.custom && taskGroups.custom.length && !isBaseTaskGroup
			? taskGroups.custom
			: taskGroups.custom.concat(taskGroups.base);
	...
	return (
		...
		>
			<InputLabel style={{ color: 'var(--fontColor)' }} id="task_group_label">
				Категория задачи
			</InputLabel>
