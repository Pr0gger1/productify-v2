...
const TaskDatesSection: FC = (): JSX.Element => {
	...
	useEffect((): void => {
		if (selectedTask) {
			if (!selectedTask.reminder) setShowReminderPicker(false);

			if (!selectedTask.deadline) setShowDeadlinePicker(false);
		}
	}, [selectedTask]);
		...
		<>
			<div className={styles.date_and_repeat}>
				{selectedTask?.deadline ?? showDeadlinePicker ? (
					<DeadlinePicker setShowDeadlinePicker={setShowDeadlinePicker} />
				) : (
					<div
						className={styles.deadline__btn}
				...
				{selectedTask?.reminder ?? showReminderPicker ? (
					<ReminderPicker setShowReminderPicker={setShowReminderPicker} />
				) : (
					<div
						className={styles.reminder__btn}
