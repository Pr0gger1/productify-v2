...
const CustomGroupContainer: FC<CustomGroupsProps> = ({
	taskGroups,
	onClick,
}) => {
	...
	const taskGroupLoading = useAppSelector(
		(state: RootState) => state.taskGroupStates.loading,
	);
	...
	return (
		<div className={styles.custom_group__container} style={hideOverflow}>
			{taskGroupLoading ? (
				<>
				...
				taskGroups.map((group: ITaskGroup) => (
					<TaskGroup
						...
						data={{
							...
							title: group.title,
							isActive:
								!isMobile && group.id === selectedTaskGroup?.id
									? 'active'
									: null,
						}}
