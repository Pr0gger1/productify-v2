...
export class DateFormatter {
	...
	getMonth(
		...
		if (!initial_form) {
			if (month.endsWith('й')) month = month.replace('й', 'я');
			else
				month.endsWith('ь')
	...
	static setDate(options: DateFormatterSetDate): number | null {
		if (options.date) {
			...
			} else if (days) return date.setDate(date.getDate() + days);
			else if (months) return date.setMonth(date.getMonth() + months);
			else return null;
		}
		return null;
	}
