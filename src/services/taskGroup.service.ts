import {getDoc, doc, setDoc, updateDoc, DocumentData} from "firebase/firestore";
...
export class TaskGroupService {
    ...
    static async updateTaskGroups(groups: ITaskGroup[], userId: string): Promise<void> {
        await updateDoc(doc(db, "tasks", userId), {
            taskGroups: groups
        });
    }

    static createCustomGroup(groups: ITaskGroup[], groupName: string): ITaskGroup[] {
    ...
    static deleteTaskGroup(groups: ITaskGroup[], groupId: string): TaskGroupsWithSelected {
        return {
            taskGroups: [...groups].filter((group: ITaskGroup) => group.id !== groupId),
            selectedTaskGroup: initialGroup
        };
    }

    static editTaskGroup(groups: ITaskGroup[], groupData: ITaskGroup): TaskGroupsWithSelected {
        let newGroups: ITaskGroup[] = [...groups];
